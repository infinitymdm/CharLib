...
class Group(Statement):
    ...
    def unique_key(self):
        ...
        if self.name == "timing" and self.attributes.get("related_pin") and self.attributes.get("timing_type"):
            return (self.name, self.attributes.get("related_pin").value, self.attributes.get("timing_type").value)
...
class Attribute(Statement):
    def __init__(self, attribute_name: str, attribute_value: bool | int | float | str | list, precision=None):
